...
Legenda: ✓ coberto por teste automatizado · ⚠ melhoria de resiliência sênior (sem teste) · ⌕ só verificável executando o app.
...
**Como foi encontrado:** teste `PostMapper.test.ts > includes every editable field in the Prisma update input` falhava com `expected undefined to be 'New title'`.
...
**Como foi encontrado:** teste `EditPostUseCase.test.ts > persists the update before publishing the PostEdited event` ⥋ esperava a ordem `['update', 'publish']` e recebia `['publish', 'update']`.
...
**Como foi encontrado:** dois testes em `resolveEventTopic.test.ts` ⥋ `'PostCreated'` retornava `'POST_EDITED'` e `'PostEdited'` retornava `'POST_CREATED'`.
...
**Como foi encontrado:** teste `getPostEventsQueueOptions.test.ts` esperava `name === ''`, `exclusive === true`, `autoDelete === true`; recebia `'posts-events-queue'`, `false`, `false`.
...
**Como foi encontrado:** teste `connections/index.test.ts > registers a single shared connection registry instance` ⥋ resolvia o token duas vezes e esperava `first === second`; recebia instâncias distintas ("serializes to the same string").
...
**Como foi encontrado:** não há teste para isto ⥋ apareceu na leitura por camada, logo após corrigir o bug 3.5. O mesmo raciocínio de ciclo de vida (transient × singleton) se aplica aqui, e o enunciado pede atenção explícita a "instâncias como singleton vs transient".
...
**Como foi encontrado:** ao ler o `onSubmit`, notei que ele inseria no cache um item com **`crypto.randomUUID()`** como `id`, enquanto a subscription `postCreated` logo abaixo tinha um guard de deduplicação que compara pelo `id` **real** do backend.
...
**Como foi encontrado:** o `useEffect` de seed do formulário tinha array de dependências **vazio** (`[]`) com um `eslint-disable`, e a query roda em modo assíncrono (`loading`).
...
Um checklist curto que teria pego **todos** os 8 bugs em revisão:
...
**Decisão desta solução:** mantivemos **persist-then-publish** (`update → publish`, com o evento derivado da entidade já confirmada). É a correção mínima, correta e sem custo operacional novo. Abaixo, o porquê, e quando graduar para o Outbox.
...
- **Honestidade sobre garantias.** Um Outbox só entrega atomicidade se a escrita do agregado e o `INSERT` no outbox compartilharem a **mesma transação**. Um Outbox "meia-boca" ⥋ dois `await` em transações separadas ⥋ carrega a complexidade do padrão **sem** eliminar o dual-write; passa uma falsa sensação de segurança. Preferimos uma solução simples e honesta a uma complexa e ilusória.
- **Escopo e perfil de perda.** Aqui os eventos alimentam atualizações de UI em tempo real (não movimentações financeiras). Perder um evento raríssimo degrada para "o usuário dá F5", não para inconsistência de dinheiro. O custo de um Outbox completo não se paga neste ponto do produto.
...
Foram corrigidos **8 problemas** distribuídos por **4 camadas** ⥋ domínio/mapeamento, use cases, injeção de dependência e a malha de eventos RabbitMQ/GraphQL no backend, mais cache e UI no frontend. Cinco tinham cobertura de teste (o sinal objetivo), dois eram bugs de runtime só visíveis usando o app, e um era uma melhoria de resiliência de nível sênior no ciclo de vida do producer.
